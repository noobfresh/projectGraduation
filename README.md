...
全时间段旅程时间折线图
一周的旅程时间折线图
统计各个时间（非计算旅程时间  和 计算的旅程时间 统计量对比）
Lstm  时间对比 准确率
...
引擎（根据输入时间起点终点 得到 旅程时间估计）
数据处理中心

需要获取相应旅程时间的前端（加个各个阶段的时间预计）

主要工作  lstm补全，画图，引擎了解，appui 数据补全


论文算法需求 算法设计 算法分析

5.26事情顺序
1。取出数据，转成csv格式，给东旭测试 ---------------finish
2。将公交数据接入！这个很重要！先把站点表整理出来，然后对接app和后台接口--------------finish
2。画web，算web要用的数据-------------doing
3。给app补个图标-----------------------finish
4。了解Lstm 

我这个东西成品是个引擎，我做了一个app demo，并且为了面对这十分高的并发量，
我需要考虑中间缓存服务器和负载均衡的设计，这个引擎背后是有一个管理后台的!

画LSTM的图：
轨交一天：一个月内（8号，15号，22号）预测（29号）
轨交一周：一个月内（1，8，15）预测（22），（2，9，16）预测（23），（3，10，17）预测（24），
（4，11，18）预测（25），（5，12，19）预测（26），（6，13，20）预测（27），
（7，14，21）预测（28）
公交一天：一个月内（5号，12号，19号） 预测（26号）
公交一周：同一线路，同一方向，一周七天，好像数据不足一周：（5，12，19 -> 26）,
