...
<div id="showPart" style="float: left;">
    <label for="dateInput" style="margin-left: 50px; margin-top: 50px; margin-bottom: 50px;">
        选择日期：</label>
    <input type="date" value="2014-05-26" id="dateInput"/>
    ...
    <input type="text" id="period" required="早/晚/其他" value="早">
...
<script type="text/javascript">
    ...
    require(
        ...
        function getOd(ec) {
            ...
            $.ajax({
                type: 'GET',
                url: "http://localhost:8080/index/admin/lstmbusday",
                data: "date=" + date + "&lineNo=" + lineNo + "&period=" + period + "&direction=" + direction,
                dataType: "json",
                success: function (result) {
                    ...
                    if (result) {
                        ...
                        for(var i = 0; i < result.result.length; i++){
                            xArray.push(result.result[i].predictBusDay.startStation + "~"
                                + result.result[i].predictBusDay.endStation);
                            true_data.push(result.result[i].predictBusDay.trueDuration);
                            predict_data.push(result.result[i].predictBusDay.predictDuration);
                        }
                        ...
                        var option = {
                            title : {
                                text: date + '日' + lineNo +'路旅程时间预测与对比（' + period + '）'
                            },
                            tooltip : {
                                trigger: 'axis'
                            },
