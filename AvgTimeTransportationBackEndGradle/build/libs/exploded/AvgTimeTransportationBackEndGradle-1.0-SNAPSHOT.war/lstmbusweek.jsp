...
<div id="showPart" style="float: left;">
    <label for="dateInput1" style="margin-left: 50px; margin-top: 50px; margin-bottom: 50px;">
        选择日期：</label>
    <input type="date" value="2014-05-26" id="dateInput1"/>
    <label for="dateInput2" style="margin-left: 10px; margin-top: 50px; margin-bottom: 50px;">
        选择日期：</label>
    <input type="date" value="2014-05-31" id="dateInput2"/>
    <label for="busInput">公交线路</label>
    <input type="text" id="busInput" style="width: 30px; margin-left: 10px" required="请输入线路号" value="108">
    <label for="direction">方向</label>
    <input type="text" id="direction" style="width: 30px; margin-left: 10px" required="R(正方向)/F(反方向)" value="R">
        <label for="start">起点：</label><input type="text" style="width: 80px; margin-left: 10px" id="start" placeholder="起点站" value="建新西路"/>
    <label for="start">终点：</label><input type="text" style="width: 80px" id="end" placeholder="终点站" value="华新街西普"/>
...
<script type="text/javascript">
    ...
    require(
        ...
        function getOd(ec) {
            ...
            $.ajax({
                type: 'GET',
                url: "http://localhost:8080/index/admin/lstmbusweek",
                data: "startDate=" + date1 + "&endDate=" +  date2 + "&lineNo=" + lineNo
                + "&direction=" + direction + "&startStation=" + startName + "&endStation=" + endName,
                dataType: "json",
                success: function (result) {
                    ...
                    if (result) {
                        ...
                        for(var i = 0; i < result.result.length; i++){
                            xArray.push(result.result[i].date);
                            true_data.push(result.result[i].trueDuration);
                            predict_data.push(result.result[i].predictDuration);
                        }
                        myChart.hideLoading();
                        var option = {
                            title : {
                                text: date1 + "~" + date2 + '间' + lineNo
                                + '路旅程时间预测与对比（'+ startName + "~" + endName + '）'
                            },
                            tooltip : {
                                trigger: 'axis'
                            },
